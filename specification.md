...
- **Portal → SCS**: for each request that needs an SCS fragment, Portal mints a short-lived signed internal token (JWT) carrying the user id and the user's roles for that SCS. The SCS verifies the signature; it does not need to know how the user originally authenticated. The internal token is signed with its own secret (`INTERNAL_TOKEN_SECRET`), separate from the browser-facing access token's secret — they protect different trust boundaries (browser-facing vs. backend-service-facing), so a leak of one must not compromise the other. Its roles claim is filtered to only the target SCS's namespaced roles (e.g. calling the `orders` SCS includes only that user's `orders:*` roles) — an SCS never learns a user's roles in unrelated SCSs.
