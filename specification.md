...
- **Route enforcement**: always performed by Portal, before any SCS is called, using the manifest's declared required role(s) for the route.
  ...
  - **Route collisions**: if two different SCSs declare the same route path, that path is excluded from the enforceable index (requests to it resolve as not-found) and the collision is reported separately so it can be logged or alerted on — a route path belongs to exactly one SCS by design, so a collision is a configuration error, never silently resolved by picking a winner.
  - **Empty `requiredRoles`**: a route declaring `"requiredRoles": []` is public — any authenticated user may access it.
  - **Access check outcomes**: for a given request path and the user's roles, the result is one of: allowed (the route matches and the user holds a required role, or `requiredRoles` is empty); not-found (no route declares this exact path); or forbidden (the route matches but the user holds none of its required roles).
